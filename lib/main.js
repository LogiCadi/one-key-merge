...
module.exports = class {
  ...
  async login() {
    ...
    await page.goto(`${this.args.url}/login/%2Fq%2Fstatus%3Aopen%2B-is%3Awip`);
  ...
  async getList() {
    const res = await axios.get(`${this.args.url}/changes/?O=881&S=0&n=25&q=status%3Aopen%20-is%3Awip`);
    const list = JSON.parse(res.data.slice(5));
    return list
      .filter((item) => this.args.target.includes(item.owner.name))
  ...
  async merge(path) {
    ...
    await page.goto(`${this.args.url}/${path}`);
    await page.waitForNetworkIdle();
